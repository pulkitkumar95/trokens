...
from trokens.utils.env import pathmgr
...
def load_orvit_boxes(cfg, mode):
    ...
    from trokens.utils.LinkBoxes.ava import get_ava_boxes
    boxes_name = get_orvit_boxes_file(cfg, mode)
    ann_filenames = [os.path.join(cfg.AVA.ANNOTATION_DIR, p) for p in boxes_name]
    return get_ava_boxes(ann_filenames)

def get_orvit_boxes_file(cfg, mode):
    if mode == 'train':
        boxes = cfg.AVA.TRAIN_GT_BOX_LISTS
    else:
        boxes = cfg.AVA.TEST_PREDICT_BOX_LISTS
