...
from trokens.datasets.multigrid_helper import ShortCycleBatchSampler
...
def detection_collate(batch):
    ...
    if isinstance(labels[0], dict):
        labels = {k:torch.tensor(np.concatenate([
                l[k] for l in labels], axis=0)).float() for k in labels[0].keys()}
    else:
...
def construct_loader(cfg, split, is_precise_bn=False, less_iters=False):
    """
...
def shuffle_dataset(loader, cur_epoch):
    ...
    ), f'Sampler type {type(sampler)} not supported'
