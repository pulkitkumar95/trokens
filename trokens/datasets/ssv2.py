...
class Ssv2(BaseDataset):
    ...
    def _construct_loader(self):
        ...
        label_file = os.path.join(self.splits_root, f'{split}/{data_split}_few_shot.json')
        with g_pathmgr.open(label_file, "r") as f:
            label_json = json.load(f)

        split_df_list = []
        num_videos = len(label_json)
        ...
        for _, video in enumerate(label_json):
            ...
            if video_name not in self.feat_vid_ids :
        ...
        num_features = len(self.split_df)
        assert num_features == num_videos, f'num_features {num_features} != num_videos {num_videos}'
